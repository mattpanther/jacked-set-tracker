...
let state = {
  month: null, workoutIdx: null, exerciseIdx: 0, pass: 1, mode: '1p',
  exerciseStates: [], // per-exercise: { ignitorReps, path, setReps, totalReps, sets, boxScore, restTime }
};
let players = []; // PlayerTracker instances

// ===== AUDIO =====
let audioReady = false, chimeSynth = null, pipSynth = null, ppDelay = null, tronSynth = null, tronFilter = null, tronReverb = null;
async function initAudio() {
  if (audioReady) return;
  try {
    await Tone.start();
    ppDelay = new Tone.PingPongDelay({ delayTime:'8n', feedback:0.6, wet:0.5 }).toDestination();
    chimeSynth = new Tone.Synth({ oscillator:{type:'sawtooth'}, envelope:{attack:0.01,decay:0.2,sustain:0.2,release:0.4} }).connect(ppDelay);
    pipSynth = new Tone.Synth({ oscillator:{type:'sine'}, envelope:{attack:0.001,decay:0.03,sustain:0,release:0.03} }).toDestination();
...
function playChime(idx) {
  if (!audioReady) return;
  const now = Tone.now();
  if (idx === 0) { chimeSynth.triggerAttackRelease('C5','16n',now); chimeSynth.triggerAttackRelease('A5','16n',now+0.15); chimeSynth.triggerAttackRelease('E6','16n',now+0.3); }
  else { chimeSynth.triggerAttackRelease('F4','16n',now); chimeSynth.triggerAttackRelease('D4','16n',now+0.15); chimeSynth.triggerAttackRelease('Bb3','16n',now+0.3); }
}
function playPip() { if (audioReady) pipSynth.triggerAttackRelease('G3','32n'); }
...
// ===== HELPERS =====
function getMonthData() { return WORKOUTS[state.month]; }
function getWorkout() { return getMonthData()?.schedule[state.workoutIdx]; }
function getAllItems() {
  const w = getWorkout(); if (!w) return [];
  return [...w.exercises, ...w.correctives.map(c => ({ ...c, isCorrective: true }))];
}
function getCurrentItem() { return getAllItems()[state.exerciseIdx]; }
function getExState(idx) {
  if (!state.exerciseStates[idx]) {
    const item = getAllItems()[idx];
    const defaultPath = item?.isCorrective ? 'corrective' :
      (state.month === 1 ? 'jacked' : 'jackedToMax');
    const mid = item?.range ? Math.round((item.range[0] + item.range[1]) / 2) : 10;
    state.exerciseStates[idx] = {
      ignitorReps: null, ignitorInput: mid, path: defaultPath,
      setReps: 0, totalReps: 0, sets: 0, boxScore: null, restTime: 90, btjActive: null,
    };
  }
...
function saveState() {
  try {
    localStorage.setItem(STATE_KEY, JSON.stringify({
      month: state.month, workoutIdx: state.workoutIdx,
      exerciseIdx: state.exerciseIdx, pass: state.pass, mode: state.mode,
    }));
  } catch(e) {}
}
function loadState() {
  try {
    ...
    if (s) { state.month = s.month; state.workoutIdx = s.workoutIdx; state.exerciseIdx = s.exerciseIdx || 0; state.pass = s.pass || 1; state.mode = s.mode || '1p'; }
  } catch(e) {}
}
...
function renderContinueCard() {
  ...
  if (state.month != null && state.workoutIdx != null) {
    ...
    if (w) {
      ...
      document.getElementById('continue-title').textContent = `Month ${state.month} — ${w.name}`;
...
function selectMonth(m) {
  ...
  document.getElementById('pass-toggle').style.display = m === 2 ? 'flex' : 'none';
  if (m === 2) {
    document.querySelectorAll('.pass-btn').forEach(b => {
      b.classList.toggle('active', parseInt(b.dataset.pass) === state.pass);
      b.onclick = () => { state.pass = parseInt(b.dataset.pass); document.querySelectorAll('.pass-btn').forEach(x => x.classList.toggle('active', parseInt(x.dataset.pass) === state.pass)); };
    });
  }
  renderDayGrid();
}
...
function renderDayGrid() {
  ...
  if (!md) { grid.innerHTML = ''; return; }
  grid.innerHTML = md.schedule.map((w, i) => {
    if (w.isRest) return `<div class="day-card rest-day" data-idx="${i}"><div class="day-card-name">😴 Rest Day</div></div>`;
    return `
  ...
  grid.querySelectorAll('.day-card').forEach(c => {
    c.onclick = () => { state.workoutIdx = parseInt(c.dataset.idx); state.exerciseIdx = 0; startTracker(); };
  });
}
...
function goBackDay() {
  const md = WORKOUTS[state.month];
  if (!md) return;
  state.workoutIdx = (state.workoutIdx - 1 + md.schedule.length) % md.schedule.length;
  state.exerciseIdx = 0;
  saveState();
  renderContinueCard();
}
...
function startTracker() {
  ...
  if (w.isRest) {
    ...
    document.getElementById('players-area').innerHTML = '<div style="text-align:center;padding:3rem;color:var(--muted);font-size:1.2rem;">😴 Rest and recover!<br><small>Use ⏮ ⏭ to navigate workouts</small></div>';
...
function setupRestNavButtons() {
  ...
  document.getElementById('next-workout-btn').onclick = () => { advanceDay(); startTracker(); };
  document.getElementById('prev-workout-btn').onclick = () => { goBackDay(); startTracker(); };
  setupModeToggle();
}
...
function renderExercise() {
  ...
  document.getElementById('exercise-count').textContent = `(${idx + 1}/${items.length})`;
...
class PlayerTracker {
  ...
  bindEvents() {
    this.dom.stepUp.onclick = () => { const es = this.getES(); es.ignitorInput++; this.dom.ignitorValue.textContent = es.ignitorInput; };
    this.dom.stepDown.onclick = () => { const es = this.getES(); es.ignitorInput = Math.max(1, es.ignitorInput - 1); this.dom.ignitorValue.textContent = es.ignitorInput; };
    this.dom.ignitorLogBtn.onclick = async () => { await initAudio(); this.logIgnitor(); };
    this.dom.pathDropdown.onchange = () => this.changePath();
    this.dom.repBtns.forEach(btn => {
      btn.onclick = async () => { await initAudio(); this.addReps(parseInt(btn.dataset.delta)); };
    });
    this.dom.commitBtn.onclick = async () => { await initAudio(); this.commitSet(); };
    this.dom.stopBtn.onclick = () => this.stopTimer();
    this.dom.resetBtn.onclick = () => this.resetExercise();
    this.dom.restBtns.forEach(btn => {
      btn.onclick = async () => {
        await initAudio(); playPip();
        const es = this.getES();
        es.restTime = parseInt(btn.dataset.time);
        this.dom.restBtns.forEach(b => b.classList.toggle('active', parseInt(b.dataset.time) === es.restTime));
      };
    });
  }

  getES() { return getExState(state.exerciseIdx); }
  ...
  renderForExercise(item, idx) {
    ...
    if (item.isCorrective) {
      ...
      dd.innerHTML = '';
      if (state.month >= 2) dd.add(new Option('🔥 Jacked to the Max', 'jackedToMax'));
  ...
  updateBoxScore(es) {
    ...
    if (useSetCount) {
      ...
      this.dom.boxRemaining.textContent = complete ? '✅ COMPLETE!' : `${targetSets - es.sets} sets remaining`;
    } else {
      // Normal box score mode
      this.dom.boxCard.querySelector('.box-score-header').textContent = 'BOX SCORE';
      ...
      this.dom.boxRemaining.textContent = complete ? '✅ COMPLETE!' : `${remaining} remaining`;
    }
  }

  isJttmSetMode(item, es) {
    return state.month === 3 && es.path === 'jackedToMax' && item?.jttmSets && !item?.isCorrective;
  }
  ...
  updateDropSet(es) {
    ...
    if (isJttM && hasDrops && md?.backToJacked) {
      ...
      if (isNow) {
        this.dom.dropBanner.querySelector('span').textContent = state.month === 3 ? '⚡ DOUBLE DROP SET NOW' : '⚡ DROP SET NOW';
        this.dom.dropBanner.classList.add('active-drop');
      } else {
        this.dom.dropBanner.querySelector('span').textContent = state.month === 3 ? '⬇️⬇️ Double Drop Set' : '⬇️ Drop Set';
  ...
  resetExercise() {
    ...
    const es = this.getES();
    const item = getCurrentItem();
    es.setReps = 0; es.totalReps = 0; es.sets = 0;
    es.ignitorReps = null; es.boxScore = null; es.btjActive = null;
    const mid = item?.range ? Math.round((item.range[0] + item.range[1]) / 2) : 10;
    es.ignitorInput = mid;
    es.path = item?.isCorrective ? 'corrective' : (state.month === 1 ? 'jacked' : 'jackedToMax');
    this.renderForExercise(item, state.exerciseIdx);
  }

  saveCurrentState() { /* state is already in exerciseStates */ }
}

function formatPath(p) {
  const map = { jacked:'Jacked', jackedUp:'Jacked Up', jackedDown:'Jacked Down', jackedToMax:'Jacked to the Max', corrective:'Corrective', pinkDB:'Pink DB', hiLoad:'Hi-Load' };
  return map[p] || p;
}
